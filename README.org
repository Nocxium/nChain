...
Move the nChain file to /usr/bin/
If you aren't a nix user you can ignore/remove the nChain.nix file.

For nix users, you can either follow the above steps, or just use the nChain.nix file with your config instead of using the regular nChain (bash) file.
*** File structure
